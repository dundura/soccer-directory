  ...
  try {
    ...
    if (resend) {
      ...
      if (review) {
        ...
        const safeClubName = escapeHtml(review.clubName);
        ...
        const commentHtml = `
          <div style="background:#f7f7f7;padding:16px;border-radius:8px;margin:16px 0;">
            <p style="color:#333;line-height:1.6;margin:0;white-space:pre-wrap;">${safeBody}</p>
          </div>
        ...
        await resend.emails.send({
          ...
          html: `
            <div style="font-family:sans-serif;max-width:600px;">
              ...
              <p><strong>${safeDisplayName}</strong> commented on the review for <strong>${safeClubName}</strong>:</p>
              <table style="width:100%;border-collapse:collapse;margin:12px 0;">
                <tr><td style="padding:4px 0;color:#666;width:120px;">Account Email</td><td style="padding:4px 0;font-weight:bold;">${safeEmail}</td></tr>
                <tr><td style="padding:4px 0;color:#666;">Account Name</td><td style="padding:4px 0;">${safeAccountName}</td></tr>
                <tr><td style="padding:4px 0;color:#666;">Display Name</td><td style="padding:4px 0;">${safeDisplayName}</td></tr>
                <tr><td style="padding:4px 0;color:#666;">User ID</td><td style="padding:4px 0;font-size:12px;color:#999;">${escapeHtml(session.user.id)}</td></tr>
              </table>
              ${commentHtml}
            </div>
          `,
            ...
            await resend.emails.send({
              ...
              html: `
                <div style="font-family:sans-serif;max-width:600px;">
                  ...
                  <p><strong>${safeDisplayName}</strong> commented on your review for <strong>${safeClubName}</strong>:</p>
                  ${commentHtml}
                </div>
              `,
            }).catch(() => {});
