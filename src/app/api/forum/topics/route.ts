...
import { createForumTopic, getForumTopics } from "@/lib/db";
...
export async function GET(req: Request) {
  try {
    ...
    const topics = await getForumTopics(category);
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const slug = await createForumTopic(title, category, body, session.user.id);

    if (resend) {
      await resend.emails.send({
        from: "Soccer Near Me <notifications@soccer-near-me.com>",
        to: NOTIFY_EMAIL,
        subject: `New Forum Topic: ${title}`,
        html: `
          <div style="font-family:sans-serif;max-width:600px;">
            <h2 style="color:#1a365d;">New Forum Discussion</h2>
            ...
            <a href="https://www.soccer-near-me.com/forum/${slug}" style="display:inline-block;padding:12px 24px;background:#DC373E;color:white;text-decoration:none;border-radius:8px;">View Topic</a>
          </div>
        `,
      }).catch(() => {});
    }
