...
export function TournamentFilters({ tournaments }: { tournaments: Tournament[] }) {
  ...
  const nonFeaturedTournaments = sorted.filter((t) => !topCardIds.has(t.id));
  const totalPages = Math.ceil(nonFeaturedTournaments.length / PER_PAGE);
  const visibleNonFeatured = viewAll ? nonFeaturedTournaments : nonFeaturedTournaments.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const hasActiveFilters = !!(state || gender || level || search);
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10">
            Browse upcoming soccer tournaments.
          </p>
          ...
          <>
            ...
            {page === 1 && !hasActiveFilters && topCards.length > 0 && (
              <div className="mb-6">
                <h2 className="font-[family-name:var(--font-display)] text-xl font-bold text-primary mb-3 uppercase tracking-wide flex items-center gap-2">
                  <span className="text-amber-500">&#9733;</span> {allFeatured.length > 0 ? "Featured Tournaments" : "Top Tournaments"}
                </h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {topCards.map((tournament) => (
                    <ListingCard
                      key={tournament.id}
                      href={`/tournaments/${tournament.slug}`}
                      title={tournament.name}
                      subtitle={`${tournament.organizer} · ${tournament.city}, ${tournament.state}`}
                      image={tournament.logo || tournament.teamPhoto || tournament.imageUrl || undefined}
                      badges={[
                        { label: tournament.level, variant: "blue" },
                        { label: tournament.gender, variant: tournament.gender === "Boys" ? "blue" : "purple" },
                      ...
                      imagePosition={tournament.imagePosition}
                                            description={tournament.description ? tournament.description.replace(/<[^>]*>/g, "").split(" ").slice(0, 25).join(" ") + (tournament.description.replace(/<[^>]*>/g, "").split(" ").length > 25 ? "..." : "") : undefined}
                      cta="View Tournament"
                    />
                  ))}
                </div>
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted font-medium">
                {nonFeaturedTournaments.length} tournament{nonFeaturedTournaments.length !== 1 ? "s" : ""} found
                {!viewAll && totalPages > 1 && <> &middot; Page {page} of {totalPages}</>}
              </p>
              {nonFeaturedTournaments.length > PER_PAGE && (
                <button
            ...
            <div className="space-y-3">
              {visibleNonFeatured.map((tournament) => {
                const img = tournament.logo || tournament.teamPhoto || tournament.imageUrl;
                return (
                  <a
                    key={tournament.id}
                    href={`/tournaments/${tournament.slug}`}
                  ...
                  >
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {tournament.dates && (
                          <p className="text-sm font-semibold text-primary mt-1">{tournament.dates}</p>
                        )}
                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">{tournament.level}</span>
                          <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{tournament.gender}</span>
                          <span className="px-3 py-1 rounded-full bg-amber-50 text-amber-700 text-xs font-medium">{tournament.format}</span>
                        </div>
                        {tournament.description && (
                          <p className="text-sm text-primary mt-2.5 line-clamp-2 hidden sm:block leading-relaxed">{tournament.description.replace(/<[^>]*>/g, "").split(" ").slice(0, 30).join(" ")}{tournament.description.replace(/<[^>]*>/g, "").split(" ").length > 30 ? "..." : ""}</p>
                        )}
                      </div>
                    </div>
