...
export function ContactYoutubeForm({ channelName, slug }: { channelName: string; slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/youtube-channels/${slug}/contact`, {
        method: "POST",
  ...
  if (sent) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-muted text-sm">Your message has been sent to {channelName}. They will contact you directly.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        <label className="block text-sm font-medium mb-1">Topic / Collaboration Idea</label>
        <input type="text" value={form.topic} onChange={(e) => setForm({ ...form, topic: e.target.value })} className={inputClass} placeholder="e.g. Skills tutorial, match analysis collab..." />
      </div>
      <div>
        <label className="block text-sm font-medium mb-1">Message <span className="text-accent">*</span></label>
        <textarea required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={inputClass + " resize-none"} placeholder="Tell the creator about your collaboration idea..." />
      </div>
