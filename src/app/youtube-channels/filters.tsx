...
const CATEGORIES = ["Youth Soccer", "Coaching", "Skills & Tutorials", "Match Analysis", "College Recruiting", "Professional Soccer", "Soccer Culture", "Fitness & Training", "Other"];
const PER_PAGE = 10;

export function YoutubeChannelFilters({ channels }: { channels: YoutubeChannel[] }) {
  ...
  const states = [...new Set(channels.map((c) => c.state))].sort();
  ...
  const nonFeatured = sorted.filter((c) => !topCardIds.has(c.id));
  ...
  return (
    <>
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <div className="bg-white rounded-2xl lg:rounded-full shadow-2xl p-2 max-w-xl mx-auto inline-flex flex-col lg:flex-row items-stretch">
            ...
            <select
              ...
              className="px-4 py-3 lg:rounded-r-full text-sm font-medium text-primary bg-transparent focus:outline-none cursor-pointer border-t lg:border-t-0 min-w-0"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        {sorted.length === 0 ? (
          <EmptyState message="No YouTube channels match your filters." />
        ) : (
          <>
            {/* ====== FEATURED CARDS ====== */}
            {page === 1 && !hasActiveFilters && topCards.length > 0 && (
              <div className="mb-10">
                <h2 className="font-[family-name:var(--font-display)] text-xl font-bold text-primary mb-5 uppercase tracking-wide flex items-center gap-2">
                  <span className="text-amber-500">&#9733;</span> {allFeatured.length > 0 ? "Featured Channels" : "Top Channels"}
                </h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {topCards.map((channel) => (
                    <ListingCard
                      key={channel.id}
                      href={`/youtube-channels/${channel.slug}`}
                      title={channel.name}
                      subtitle={`${channel.creatorName} · ${channel.city}, ${channel.state}`}
                      image={channel.logo || channel.teamPhoto || channel.imageUrl || undefined}
                      badges={[
                        { label: channel.category, variant: "blue" },
                      ]}
                      details={[
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted font-medium">
                {nonFeatured.length} channel{nonFeatured.length !== 1 ? "s" : ""} found
            ...
            <div className="space-y-3">
              {visibleNonFeatured.map((channel) => {
                const img = channel.logo || channel.teamPhoto || channel.imageUrl;
                return (
                  <a
                    key={channel.id}
                    href={`/youtube-channels/${channel.slug}`}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center flex-shrink-0 p-2 sm:p-4">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-lg overflow-hidden bg-surface flex items-center justify-center">
                        {img ? (
                          <img src={img} alt={channel.name} className="w-full h-full object-contain" />
                        ) : (
                          <svg className="w-10 h-10 text-muted/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.347a1.125 1.125 0 010 1.972l-11.54 6.347a1.125 1.125 0 01-1.667-.986V5.653z" /></svg>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">{channel.category}</span>
                          <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{channel.creatorName}</span>
                        </div>
                        {channel.description && (
                          <p className="text-sm text-primary mt-2.5 line-clamp-2 hidden sm:block leading-relaxed">
                            {channel.description.split(/\s+/).slice(0, 30).join(" ")}{channel.description.split(/\s+/).length > 30 ? "..." : ""}
                          </p>
                        )}
                      </div>
                    </div>
