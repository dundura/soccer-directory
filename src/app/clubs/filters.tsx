...
export function ClubFilters({ clubs }: { clubs: Club[] }) {
  ...
  const nonFeaturedClubs = sorted.filter((c) => !topCardIds.has(c.id));
  const totalPages = Math.ceil(nonFeaturedClubs.length / PER_PAGE);
  const visibleNonFeatured = viewAll ? nonFeaturedClubs : nonFeaturedClubs.slice((page - 1) * PER_PAGE, page * PER_PAGE);
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10">
            Browse youth soccer clubs in your area.
          </p>
          ...
          <>
            ...
            {topCards.length > 0 && (
              <div className="mb-10">
                <h2 className="font-[family-name:var(--font-display)] text-xl font-bold text-primary mb-5 uppercase tracking-wide flex items-center gap-2">
                  <span className="text-amber-500">&#9733;</span> {allFeatured.length > 0 ? "Featured Clubs" : "Top Clubs"}
                </h2>
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted font-medium">
                {nonFeaturedClubs.length} club{nonFeaturedClubs.length !== 1 ? "s" : ""} found
                {!viewAll && totalPages > 1 && <> &middot; Page {page} of {totalPages}</>}
              </p>
              {nonFeaturedClubs.length > PER_PAGE && (
                <button
                  ...
                  >
                    ...
                    <div className="hidden sm:flex items-center justify-center flex-shrink-0 p-3 sm:p-4">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-lg overflow-hidden bg-surface flex items-center justify-center">
                        {img ? (
                          <img src={img} alt={club.name} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {club.description && (
                          <p className="text-sm text-primary mt-2.5 line-clamp-2 hidden sm:block leading-relaxed">{club.description}</p>
                        )}
                      </div>
                    </div>
