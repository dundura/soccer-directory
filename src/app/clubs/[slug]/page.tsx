...
const plainDescription = club.description
    ? club.description.replace(/<[^>]*>/g, " ").replace(/s+/g, " ").trim()
    : `Youth soccer club in ${club.city}, ${club.state}`;
    return ogMeta(
      ...
      <div className="max-w-[1100px] mx-auto px-6 pb-16">
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-5 lg:gap-6 items-start">
          ...
          <aside className="order-4 lg:order-none lg:[grid-row:span_10] flex flex-col gap-4">
            ...
            {/* Special Events */}
            <ListingEventsSidebar listingType="club" listingId={String(club.id)} listingSlug={slug} ownerId={ownerId} />
          </aside>
          ...
          <div className="order-1 lg:order-none lg:col-start-2 bg-white rounded-2xl overflow-hidden shadow-sm">
              ...
              <div>
                ...
                {club.description && (
                  <div className="mb-0">
                    <InlineEditField ownerId={ownerId} listingType="club" listingId={club.id} field="description" value={club.description} tag="div" className="text-sm leading-relaxed text-gray-500 whitespace-pre-line [&>p]:mb-2 [&>p:last-child]:mb-0" multiline />
                  </div>
                )}
