...
export function ScrimmageFilters({ scrimmages }: { scrimmages: Scrimmage[] }) {
  ...
  const states = [...new Set(scrimmages.map((s) => s.state))].sort();
  ...
  const nonFeaturedScrimmages = sorted.filter((s) => !topCardIds.has(s.id));
  const totalPages = Math.ceil(nonFeaturedScrimmages.length / PER_PAGE);
  const visibleNonFeatured = viewAll ? nonFeaturedScrimmages : nonFeaturedScrimmages.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const hasActiveFilters = !!(search || level || ageGroup || state || gender || availability);
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10">
            Find teams looking for scrimmage partners.
          </p>
          ...
          <div className="bg-white rounded-2xl lg:rounded-full shadow-2xl p-2 max-w-4xl mx-auto">
            <div className="flex flex-col lg:flex-row items-stretch">
              ...
              >
                ...
                <option value="Looking for Scrimmage">Looking for Scrimmage</option>
                <option value="Open to Scrimmages">Open to Scrimmages</option>
                <option value="Looking for Regular Scrimmage Partners">Regular Partners</option>
              </select>
              <button
                type="button"
          ...
          <>
            ...
            {page === 1 && !hasActiveFilters && topCards.length > 0 && (
              <div className="mb-6">
                <h2 className="font-[family-name:var(--font-display)] text-xl font-bold text-primary mb-3 uppercase tracking-wide flex items-center gap-2">
                  <span className="text-amber-500">&#9733;</span> {allFeatured.length > 0 ? "Featured Scrimmages" : "Top Scrimmages"}
                </h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {topCards.map((s) => (
                    <ListingCard
                      ...
                      image={s.logo || s.teamPhoto || s.imageUrl || undefined}
                      badges={[
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted font-medium">
                {nonFeaturedScrimmages.length} scrimmage{nonFeaturedScrimmages.length !== 1 ? "s" : ""} found
                {!viewAll && totalPages > 1 && <> &middot; Page {page} of {totalPages}</>}
              </p>
              {nonFeaturedScrimmages.length > PER_PAGE && (
                <button
            ...
            <div className="space-y-3">
              {visibleNonFeatured.map((s) => {
                const img = s.logo || s.teamPhoto || s.imageUrl;
                return (
                  <a
                    key={s.id}
                    href={`/scrimmages/${s.slug}`}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center flex-shrink-0 p-2 sm:p-4">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-lg overflow-hidden bg-surface flex items-center justify-center">
                        {img ? (
                          <img src={img} alt={s.teamName} className="w-full h-full object-contain" />
                        ) : (
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">{s.level}</span>
                          <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{s.ageGroup}</span>
                          <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{s.gender}</span>
                          <span className="px-3 py-1 rounded-full bg-green-50 text-green-700 text-xs font-semibold">{s.availability}</span>
                        </div>
                        {s.description && (
