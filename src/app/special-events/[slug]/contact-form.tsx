...
export function ContactSpecialEventForm({ eventName, slug }: { eventName: string; slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({ type: "specialevent", slug, ...form, captchaToken }),
      });
      if (!res.ok) {
  ...
  if (sent) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-muted text-sm">Your message has been sent to the organizer of {eventName}.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={inputClass + " resize-none"} placeholder="Ask about event details, registration, etc..." />
      </div>
