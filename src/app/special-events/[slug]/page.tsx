...
import { getSpecialEventBySlug, getSpecialEventSlugs, getListingOwner } from "@/lib/db";
...
export async function generateStaticParams() {
  try {
    const slugs = await getSpecialEventSlugs();
...
export default async function SpecialEventDetailPage({ params }: Props) {
  const { slug } = await params;

  let event;
  try {
    event = await getSpecialEventBySlug(slug);
  } catch {
    throw new Error("Failed to load event details. Please try again later.");
  }
  if (!event) notFound();

  let ownerId: string | null = null;
  try {
    ownerId = await getListingOwner("specialevent", slug);
  } catch {
    ownerId = null;
  }

  const pageUrl = `https://www.soccer-near-me.com/special-events/${slug}`;
  const imgPos = event.imagePosition ?? 50;
  const heroPos = event.heroImagePosition ?? 50;
  const heroPhoto = event.imageUrl || event.teamPhoto || DEFAULT_HERO_PHOTO;
  const eventPhotos = event.photos && event.photos.length > 0 ? event.photos : DEFAULT_PHOTOS;
  const logo = event.logo || DEFAULT_LOGO;
  const videoUrl = event.videoUrl === undefined || event.videoUrl === null ? DEFAULT_VIDEO : event.videoUrl || null;
  ...
  return (
    <>
      ...
      <div className="max-w-[1100px] mx-auto px-6 py-3.5 text-sm text-muted flex items-center justify-between">
        <div>
          <a href="/special-events" className="text-primary hover:underline">Special Events</a>
          {" \u203A "}
          <span>{event.state}</span>
          {" \u203A "}
          <span>{event.name}</span>
        </div>
        <ManageListingButton ownerId={ownerId} listingType="specialevent" listingId={event.id} />
      </div>
        ...
        <div className="grid lg:grid-cols-[280px_1fr] gap-6 items-start">
          ...
          <aside className="flex flex-col gap-4 order-2 lg:order-1">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              <img src={event.teamPhoto || DEFAULT_SIDEBAR_PHOTO} alt={event.name} className="w-full h-[200px] object-cover block" style={{ objectPosition: `center ${imgPos}%` }} />
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              {[
                ...
                { label: "Gender", value: event.gender },
                ...(event.cost ? [{ label: "Cost", value: event.cost }] : []),
                { label: "Contact", value: event.organizerName },
                ...(event.clubName ? [{ label: "Club", value: event.clubName }] : []),
              ...
              {event.socialMedia && (event.socialMedia.facebook || event.socialMedia.instagram) && (
                <div className="flex justify-center gap-4 py-3.5 border-t border-border">
                  {event.socialMedia.instagram && (
                    <a href={event.socialMedia.instagram} target="_blank" className="w-[34px] h-[34px] rounded-full bg-surface flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-colors" title="Instagram">
                  ...
                  {event.socialMedia.facebook && (
                    <a href={event.socialMedia.facebook} target="_blank" className="w-[34px] h-[34px] rounded-full bg-surface flex items-center justify-center text-primary hover:bg-primary hover:text-white transition-colors" title="Facebook">
              ...
              <div className="bg-white rounded-2xl p-[18px] shadow-sm space-y-3.5">
                {event.phone && (
                  <div>
                    ...
                    <p className="text-sm font-bold text-primary">{event.phone}</p>
                  </div>
                )}
                {event.email && (
                  <div>
          ...
          <main className="flex flex-col gap-5 order-1 lg:order-2">
            ...
            {/* Hero */}
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              <HeroImage src={heroPhoto} alt={event.name} id={event.id} imagePosition={heroPos} />
              <div className="p-5 sm:p-7 sm:flex sm:gap-6 sm:items-start">
                ...
                <div className="mt-3 sm:mt-0 sm:flex-1 sm:min-w-0 relative z-20">
                  <InlineEditField ownerId={ownerId} listingType="specialevent" listingId={event.id} field="name" value={event.name} tag="h1" className="text-xl sm:text-[26px] font-extrabold text-primary leading-tight tracking-tight" />
                  {event.tagline && (
                    <InlineEditField ownerId={ownerId} listingType="specialevent" listingId={event.id} field="tagline" value={event.tagline} tag="p" className="text-sm text-accent font-medium mt-1" />
                  )}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-2.5 mt-1">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-bold text-primary ml-auto">{event.eventType}</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">&#128197;</span>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Dates</span>
                  <span className="text-sm font-bold text-primary ml-auto">{event.dates}</span>
                </div>
                {event.time && (
                  <div className="flex items-center gap-2.5">
                    <span className="text-lg leading-none">&#128336;</span>
                    <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Time</span>
                    <span className="text-sm font-bold text-primary ml-auto">{event.time}</span>
                  </div>
                )}
                {event.cost && (
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm font-bold text-primary ml-auto">{event.cost}</span>
                  </div>
                )}
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">&#127874;</span>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Ages</span>
                  <span className="text-sm font-bold text-primary ml-auto">{event.ageGroup}</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">&#9917;</span>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Gender</span>
                  <span className="text-sm font-bold text-primary ml-auto">{event.gender}</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">&#128205;</span>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Location</span>
                  <span className="text-sm font-bold text-primary ml-auto">{event.location || `${event.city}, ${event.state}`}</span>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className={`grid grid-cols-2 gap-2.5 ${videoUrl ? "mb-4" : ""}`}>
                {eventPhotos.map((photo, i) => (
                  <img key={i} src={photo} alt={`Event photo ${i + 1}`} className="w-full aspect-square object-cover rounded-xl block" />
                ))}
              </div>
