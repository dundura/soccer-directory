...
export function ContactGroupForm({ groupName, slug }: { groupName: string; slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/facebook-groups/${slug}/contact`, {
        method: "POST",
  ...
  if (sent) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-muted text-sm">Your message has been sent to the admin of {groupName}. They will contact you directly.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={inputClass + " resize-none"} placeholder="Ask a question about this group..." />
      </div>
