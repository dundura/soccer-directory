...
export function RecruiterFilters({ recruiters }: { recruiters: Recruiter[] }) {
  ...
  const nonFeaturedRecruiters = sorted.filter((t) => !topCardIds.has(t.id));
  const totalPages = Math.ceil(nonFeaturedRecruiters.length / PER_PAGE);
  const visibleNonFeatured = viewAll ? nonFeaturedRecruiters : nonFeaturedRecruiters.slice((page - 1) * PER_PAGE, page * PER_PAGE);

  const hasActiveFilters = !!(search || specialty || state);
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10">
            Find college soccer recruiters and services.
          </p>
          ...
          <>
            ...
            {page === 1 && !hasActiveFilters && topCards.length > 0 && (
              <div className="mb-6">
                <h2 className="font-[family-name:var(--font-display)] text-xl font-bold text-primary mb-3 uppercase tracking-wide flex items-center gap-2">
                  <span className="text-amber-500">&#9733;</span> {allFeatured.length > 0 ? "Featured Recruiters" : "Top Recruiters"}
                </h2>
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {topCards.map((recruiter) => (
                    <ListingCard
                      key={recruiter.id}
                      ...
                      badges={[
                        ...(recruiter.specialty ? [{ label: recruiter.specialty, variant: "green" as const }] : []),
                      ]}
                      details={[
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted font-medium">
                {nonFeaturedRecruiters.length} advisor{nonFeaturedRecruiters.length !== 1 ? "s" : ""} found
                {!viewAll && totalPages > 1 && <> &middot; Page {page} of {totalPages}</>}
              </p>
              {nonFeaturedRecruiters.length > PER_PAGE && (
                <button
            ...
            <div className="space-y-3">
              {visibleNonFeatured.map((recruiter) => {
                const img = recruiter.logo || recruiter.teamPhoto || recruiter.imageUrl;
                return (
                  <a
                    key={recruiter.id}
                    href={`/college-recruiting/${recruiter.slug}`}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center flex-shrink-0 p-2 sm:p-4">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-lg overflow-hidden bg-surface flex items-center justify-center">
                        {img ? (
                          <img src={img} alt={recruiter.name} className="w-full h-full object-contain" />
                        ) : (
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                          {recruiter.specialty && (
                            <span className="px-3 py-1 rounded-full bg-green-50 text-green-700 text-xs font-semibold">{recruiter.specialty}</span>
                          )}
                          {recruiter.serviceArea && (
                            <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{recruiter.serviceArea.length > 40 ? recruiter.serviceArea.slice(0, 40) + "..." : recruiter.serviceArea}</span>
                          )}
                          {recruiter.priceRange && (
                            <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{recruiter.priceRange}</span>
                          )}
                        </div>
