...
import { getPhotoVideoServiceBySlug, getPhotoVideoServiceSlugs, getListingOwner } from "@/lib/db";
...
export async function generateStaticParams() {
  try {
    const slugs = await getPhotoVideoServiceSlugs();
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const service = await getPhotoVideoServiceBySlug(slug);
  if (!service) return {};
...
export default async function ServiceDetailPage({ params }: Props) {
  ...
  let service;
  try {
    service = await getPhotoVideoServiceBySlug(slug);
  } catch {
  ...
  try {
    ownerId = await getListingOwner("photovideo", slug);
  } catch {
    ownerId = null;
  }

  const pageUrl = `https://www.soccer-near-me.com/photo-video-services/${slug}`;
  ...
  return (
    <>
      ...
      <div className="max-w-4xl mx-auto px-6 py-3.5 text-sm text-muted flex items-center justify-between">
        <div>
          <a href="/photo-video-services" className="text-primary hover:underline">Products &amp; Services</a>
          {" \u203A "}
          <span>{service.category}</span>
          {" \u203A "}
          <span>{service.name}</span>
        </div>
        <ManageListingButton ownerId={ownerId} listingType="photovideo" listingId={service.id} />
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 pb-16">
          ...
          <div className="p-6 md:p-8">
            <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
              <div>
                ...
                <InlineEditField ownerId={ownerId} listingType="photovideo" listingId={service.id} field="name" value={service.name} tag="h1" className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-primary uppercase leading-tight tracking-tight" />
                {service.tagline && (
                  <InlineEditField ownerId={ownerId} listingType="photovideo" listingId={service.id} field="tagline" value={service.tagline} tag="p" className="text-sm text-accent font-medium mt-1" />
                )}
            ...
            {service.description && (
              <div className="mb-6">
                <InlineEditField ownerId={ownerId} listingType="photovideo" listingId={service.id} field="description" value={service.description} tag="div" className="text-gray-600 leading-relaxed whitespace-pre-line [&>p]:mb-2 [&>p:last-child]:mb-0" multiline />
              </div>
            )}
            ...
            <div className="border-t border-border pt-6 grid grid-cols-2 gap-4 text-sm">
              ...
              {service.email && (
                <div>
                  ...
                  <p className="font-bold text-accent-hover"><a href={`/contact/photovideo/${slug}`} className="hover:underline">Contact</a></p>
                </div>
              )}
              {service.phone && (
                <div>
            ...
            {service.aboutAuthor && (
              <div className="border-t border-border pt-6 mt-6">
                ...
                <InlineEditField ownerId={ownerId} listingType="photovideo" listingId={service.id} field="aboutAuthor" value={service.aboutAuthor} tag="p" className="text-gray-600 leading-relaxed whitespace-pre-line" multiline />
              </div>
            )}
        ...
        <Suspense fallback={<div className="bg-white rounded-2xl p-6 shadow-sm"><div className="h-5 w-24 bg-gray-200 rounded animate-pulse mb-4" /><div className="h-20 bg-gray-200 rounded animate-pulse" /></div>}>
          <ReviewSection listingType="photovideo" listingId={service.id} />
        </Suspense>
