  ...
  const sidebarContent = (
    <>
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
        <ClickableImage src={camp.previewImage || camp.teamPhoto || DEFAULT_SIDEBAR_PHOTO} alt={camp.name} className="w-full min-h-[150px] max-h-[280px] object-contain bg-surface block p-2" style={{ objectPosition: `center ${imgPos}%` }} />
        <div className="text-center py-3.5 px-4">
          <h3 className="font-[family-name:var(--font-display)] text-lg sm:text-xl font-extrabold text-primary uppercase tracking-tight leading-snug">{camp.name}</h3>
          ...
          <main className="flex flex-col gap-5">
            ...
            {camp.sponsors && camp.sponsors.length > 0 && (
              <div className="order-8 lg:order-none lg:col-start-2">
                <SponsorsSection sponsors={camp.sponsors} />
              </div>
            )}
