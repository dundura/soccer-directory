  ...
  const sidebarContent = (
    <>
      ...
      <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
        <ClickableImage src={trainer.previewImage || trainer.teamPhoto || DEFAULT_SIDEBAR_PHOTO} alt={trainer.name} className="w-full min-h-[150px] max-h-[280px] object-contain bg-surface block p-2" style={{ objectPosition: `center ${imgPos}%` }} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-5 lg:gap-6 items-start">
          ...
          <main className="flex flex-col gap-5">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              ...
              {/* Logo stacked ABOVE the text, not beside it. Side by side, the
                  logo's width plus the gap indented the name, the description
                  and everything under it by ~96px, so nothing in this card
                  lined up with At a Glance or Background & Credentials below.
                  Stacked, the text starts at the card's left edge like the rest
                  of the page, and the logo can ride higher into the hero. */}
              <div className="p-5 sm:p-7">
                <img
                  src={logo}
                  alt={`${trainer.name} logo`}
                  className="w-[56px] h-[56px] sm:w-[72px] sm:h-[72px] rounded-xl border-2 border-border object-contain shrink-0 p-1 sm:p-1.5 bg-surface -mt-11 sm:-mt-14 mb-3 relative z-10"
                />
                <div className="min-w-0">
                  ...
                  {trainer.description && (
                    <div className="mb-0">
                      <InlineEditField ownerId={ownerId} listingType="trainer" listingId={trainer.id} field="description" value={trainer.description} tag="div" className="text-sm leading-relaxed text-gray-500 whitespace-pre-line [&>p]:mb-2 [&>p:last-child]:mb-0" multiline />
                    </div>
                  )}
            ...
            {/* Featured Articles */}
            {/* Events */}
            <ListingEventsSection listingType="trainer" listingId={trainer.id} listingSlug={slug} ownerId={ownerId} />
