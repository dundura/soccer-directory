...
export function ContactPageForm({ pageName, slug }: { pageName: string; slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/instagram-pages/${slug}/contact`, {
        method: "POST",
  ...
  if (sent) {
    return (
      <div className="text-center py-6">
        ...
        <p className="text-muted text-sm">Your message has been sent to the owner of {pageName}. They will contact you directly.</p>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={inputClass + " resize-none"} placeholder="Ask a question about this Instagram page..." />
      </div>
