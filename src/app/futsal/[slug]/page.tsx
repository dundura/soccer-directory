...
const plainDescription = team.description
    ? team.description.replace(/<[^>]*>/g, " ").replace(/s+/g, " ").trim()
    : `${team.level} futsal team for ${team.ageGroup} ${team.gender} in ${team.city}, ${team.state}`;
    return ogMeta(
  ...
  return (
    <>
      ...
      <div className="max-w-[1100px] mx-auto px-6 py-3.5 text-sm text-muted flex items-center justify-between">
        <div>
          <a href="/futsal" className="text-primary hover:underline">Futsal</a>
          {" \u203A "}
          <span>{team.state}</span>
          {" \u203A "}
          <span>{team.name}</span>
        </div>
        <ManageListingButton ownerId={ownerId} listingType="futsal" listingId={team.id} />
      </div>
          ...
          <aside className="flex flex-col gap-4 order-2 lg:order-1">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              {[
                ...
                { label: "Format", value: team.format },
          ...
          <main className="flex flex-col gap-5 order-1 lg:order-2">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              ...
              <div className="flex-1 min-w-0">
                <InlineEditField ownerId={ownerId} listingType="futsal" listingId={team.id} field="name" value={team.name} tag="h1" className="text-2xl sm:text-3xl md:text-4xl font-extrabold text-primary uppercase leading-tight tracking-tight" />
                {team.tagline && (
                ...
                {team.description && (
                  <div className="mb-0">
                    <InlineEditField ownerId={ownerId} listingType="futsal" listingId={team.id} field="description" value={team.description} tag="div" className="text-sm leading-relaxed text-gray-500 whitespace-pre-line [&>p]:mb-2 [&>p:last-child]:mb-0" multiline />
                  </div>
                )}
            ...
            <div className="bg-white rounded-2xl p-6 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-2.5 mt-1">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-bold text-primary ml-auto">{team.format}</span>
                </div>
                <div className="flex items-center gap-2.5">
                  <span className="text-lg leading-none">&#127941;</span>
                  <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Season</span>
                ...
                {team.positionsNeeded && (
                  <div className="flex items-center gap-2.5">
                    <span className="text-lg leading-none">&#128101;</span>
                    <span className="text-[11px] font-bold uppercase tracking-wider text-gray-400">Positions</span>
                    <span className="text-sm font-bold text-primary ml-auto">{team.positionsNeeded}</span>
                  </div>
                )}
              </div>
            </div>
