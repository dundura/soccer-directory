...
const plainDescription = tryout.description
    ? tryout.description.replace(/<[^>]*>/g, " ").replace(/s+/g, " ").trim()
    : `Soccer tryout in ${tryout.city}, ${tryout.state}`;
    return ogMeta(
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-5 lg:gap-6 items-start">
          ...
          <aside className="order-4 lg:order-none lg:[grid-row:span_10] flex flex-col gap-4">
            ...
            <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
              <ClickableImage src={tryout.previewImage || tryout.teamPhoto || DEFAULT_SIDEBAR_PHOTO} alt={tryout.name} className="w-full min-h-[150px] max-h-[280px] object-contain bg-surface block p-2" style={{ objectPosition: `center ${imgPos}%` }} />
          ...
          {/* ====== RIGHT MAIN COLUMN ====== */}
            ...
            <div className="order-6 lg:order-none lg:col-start-2">
              {/* Events */}
              <ListingEventsSection listingType="tryout" listingId={tryout.id} listingSlug={slug} ownerId={ownerId} />
