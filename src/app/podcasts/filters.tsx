...
export function PodcastFilters({ podcasts }: { podcasts: Podcast[] }) {
  ...
  const categories = [...new Set(podcasts.map((p) => p.category))].sort();
  ...
  const nonFeaturedPodcasts = sorted.filter((p) => !topCardIds.has(p.id));
  ...
  return (
    <>
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10">
            Discover soccer podcasts and shows.
          </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        {sorted.length === 0 ? (
          <EmptyState message="No podcasts match your filters." />
        ) : (
          <>
            {/* ====== FEATURED CARDS ====== */}
            ...
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm text-muted font-medium">
                {nonFeaturedPodcasts.length} podcast{nonFeaturedPodcasts.length !== 1 ? "s" : ""} found
                {!viewAll && totalPages > 1 && <> &middot; Page {page} of {totalPages}</>}
              </p>
              {nonFeaturedPodcasts.length > PER_PAGE && (
                <button
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center flex-shrink-0 p-2 sm:p-4">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-lg overflow-hidden bg-surface flex items-center justify-center">
                        {img ? (
                          <img src={img} alt={podcast.name} className={`w-full h-full ${isLogo ? "object-contain" : "object-cover"}`} />
                        ) : (
                          <svg className="w-10 h-10 text-muted/20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m-4 0h8m-4-18a3 3 0 00-3 3v4a3 3 0 006 0V7a3 3 0 00-3-3z" /></svg>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted flex items-center gap-1.5 mt-1">
                          ...
                          {podcast.city}, {podcast.state}
                        </p>
                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">{podcast.category}</span>
                          {podcast.hostName && (
                            <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{podcast.hostName}</span>
                          )}
                        </div>
                        {podcast.description && (
