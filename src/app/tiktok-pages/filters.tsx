...
export function TikTokPageFilters({ pages }: { pages: TikTokPage[] }) {
  ...
  const states = [...new Set(pages.map((p) => p.state))].sort();
  ...
  return (
    <>
      ...
      <div className="relative bg-primary overflow-hidden">
        ...
        <div className="relative max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 text-center">
          ...
          <p className="text-white/60 text-base md:text-lg max-w-2xl mx-auto mb-10">
            Follow soccer creators on TikTok.
          </p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        ...
        {sorted.length === 0 ? (
          <EmptyState message="No TikTok pages match your filters." />
        ) : (
          <>
            {/* ====== FEATURED CARDS ====== */}
              ...
              <div className="mb-6">
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {topCards.map((tkPage) => (
                    <ListingCard
                      key={tkPage.id}
                      href={`/tiktok-pages/${tkPage.slug}`}
                      title={tkPage.name}
                      subtitle={`${tkPage.city}, ${tkPage.state}`}
                      image={tkPage.logo || tkPage.teamPhoto || undefined}
                      badges={[
                        { label: tkPage.category, variant: "blue" },
                      ]}
                      details={[
            ...
            <div className="space-y-3">
              {visibleNonFeatured.map((tkPage) => {
                const img = tkPage.logo || tkPage.teamPhoto;
                return (
                  <a
                    key={tkPage.id}
                    href={`/tiktok-pages/${tkPage.slug}`}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-center flex-shrink-0 p-2 sm:p-4">
                      <div className="w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 rounded-lg overflow-hidden bg-surface flex items-center justify-center">
                        {img ? (
                          <img src={img} alt={tkPage.name} className="w-full h-full object-contain" />
                        ) : (
                          <svg className="w-10 h-10 text-muted/20" fill="currentColor" viewBox="0 0 24 24"><path d="M19.59 6.69a4.83 4.83 0 01-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 01-2.88 2.5 2.89 2.89 0 01-2.89-2.89 2.89 2.89 0 012.89-2.89c.28 0 .54.04.79.1v-3.5a6.37 6.37 0 00-.79-.05A6.34 6.34 0 003.15 15.2a6.34 6.34 0 006.34 6.34 6.34 6.34 0 006.34-6.34V8.72a8.2 8.2 0 004.77 1.52V6.79a4.85 4.85 0 01-1.01-.1z"/></svg>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-start gap-5 sm:gap-6 flex-1 min-w-0 p-5 sm:p-6">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted mt-1">
                          {tkPage.city}, {tkPage.state}
                        </p>
                        <div className="flex flex-wrap items-center gap-1.5 mt-2">
                          <span className="px-3 py-1 rounded-full bg-blue-50 text-blue-700 text-xs font-semibold">{tkPage.category}</span>
                          {tkPage.followerCount && (
                            <span className="px-3 py-1 rounded-full bg-surface text-muted text-xs font-medium">{tkPage.followerCount} followers</span>
                          )}
                        </div>
                        {tkPage.description && (
