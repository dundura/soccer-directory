...
export function ContactPageForm({ pageName, slug }: { pageName: string; slug: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`/api/tiktok-pages/${slug}/contact`, {
        method: "POST",
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea required rows={4} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className={inputClass + " resize-none"} placeholder="Ask a question about this TikTok page..." />
      </div>
