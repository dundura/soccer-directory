...
export async function createClubListing(data: Record<string, string>, userId: string) {
  ...
  await sql`INSERT INTO clubs (id, slug, name, city, country, state, level, league, league_url, age_groups, gender, team_count, description, website, email, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, practice_schedule, address, media_links, announcement_heading, announcement_text, announcement_image, announcement_heading_2, announcement_text_2, announcement_image_2, announcement_heading_3, announcement_text_3, announcement_image_3, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.level}, ${data.league || null}, ${data.leagueUrl || null}, ${data.ageGroups}, ${data.gender}, ${Number(data.teamCount) || 0}, ${data.description}, ${data.website || null}, ${data.email || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.practiceSchedule}, ${pf.address}, ${pf.mediaLinks}, ${data.announcementHeading || null}, ${data.announcementText || null}, ${data.announcementImage || null}, ${data.announcementHeading2 || null}, ${data.announcementText2 || null}, ${data.announcementImage2 || null}, ${data.announcementHeading3 || null}, ${data.announcementText3 || null}, ${data.announcementImage3 || null}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createTeamListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO teams (id, slug, name, club_name, city, country, state, level, age_group, gender, coach, looking_for_players, positions_needed, season, description, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, practice_schedule, address, events, annual_tournaments, media_links, announcement_heading, announcement_text, announcement_image, announcement_heading_2, announcement_text_2, announcement_image_2, announcement_heading_3, announcement_text_3, announcement_image_3, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.clubName || null}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.level}, ${data.ageGroup}, ${data.gender}, ${data.coach}, ${data.lookingForPlayers === "true"}, ${data.positionsNeeded || null}, ${data.season}, ${data.description || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.practiceSchedule}, ${pf.address}, ${data.events || null}, ${data.annualTournaments || null}, ${pf.mediaLinks}, ${data.announcementHeading || null}, ${data.announcementText || null}, ${data.announcementImage || null}, ${data.announcementHeading2 || null}, ${data.announcementText2 || null}, ${data.announcementImage2 || null}, ${data.announcementHeading3 || null}, ${data.announcementText3 || null}, ${data.announcementImage3 || null}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createTrainerListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO trainers (id, slug, name, city, country, state, specialty, experience, credentials, price_range, service_area, description, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, practice_schedule, address, rating, review_count, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.specialty}, ${data.experience}, ${data.credentials}, ${data.priceRange}, ${data.serviceArea}, ${data.description || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.practiceSchedule}, ${pf.address}, 0, 0, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createCampListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO camps (id, slug, name, organizer_name, city, country, state, camp_type, age_range, dates, price, gender, location, description, registration_url, email, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.organizerName}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.campType}, ${data.ageRange}, ${data.dates}, ${data.price}, ${data.gender}, ${data.location || null}, ${data.description}, ${data.registrationUrl || null}, ${data.email || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createGuestListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO guest_opportunities (id, slug, team_name, city, country, state, level, age_group, gender, dates, tournament, positions_needed, contact_email, description, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, featured, user_id, status) VALUES (${id}, ${slug}, ${data.teamName}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.level}, ${data.ageGroup}, ${data.gender}, ${data.dates}, ${data.tournament}, ${data.positionsNeeded}, ${data.contactEmail}, ${data.description || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createTournamentListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO tournaments (id, slug, name, organizer, city, country, state, dates, age_groups, gender, level, entry_fee, format, description, registration_url, email, region, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.organizer}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.dates}, ${data.ageGroups}, ${data.gender}, ${data.level}, ${data.entryFee}, ${data.format}, ${data.description}, ${data.registrationUrl || null}, ${data.email || null}, ${data.region || 'US'}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createFutsalListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO futsal_teams (id, slug, name, club_name, city, country, state, level, age_group, gender, coach, looking_for_players, positions_needed, season, description, format, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, practice_schedule, address, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.clubName || null}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.level}, ${data.ageGroup}, ${data.gender}, ${data.coach}, ${data.lookingForPlayers === "true"}, ${data.positionsNeeded || null}, ${data.season}, ${data.description || null}, ${data.format || '5v5'}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.practiceSchedule}, ${pf.address}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createTripListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO international_trips (id, slug, trip_name, organizer, destination, city, country, state, dates, age_group, gender, level, price, spots_available, contact_email, description, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, featured, user_id, status) VALUES (${id}, ${slug}, ${data.tripName}, ${data.organizer}, ${data.destination}, ${data.city}, ${data.country || 'International'}, ${data.state || ''}, ${data.dates}, ${data.ageGroup}, ${data.gender}, ${data.level}, ${data.price || null}, ${data.spotsAvailable || null}, ${data.contactEmail}, ${data.description || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createMarketplaceListing(data: Record<string, string>, userId: string) {
  const id = genId();
  const slug = slugify(data.name);
  await sql`INSERT INTO marketplace (id, slug, name, category, description, price, condition, city, country, state, contact_email, phone, image_url, image_position, photos, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.category}, ${data.description}, ${data.price}, ${data.condition}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.contactEmail}, ${data.phone || null}, ${data.imageUrl || null}, ${Number(data.imagePosition) || 50}, ${data.photos || null}, false, ${userId}, 'pending')`;
  return slug;
}
...
export async function createPlayerProfile(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO player_profiles (id, slug, player_name, position, secondary_position, birth_year, height, preferred_foot, current_club, city, country, state, level, gender, gpa, description, looking_for, contact_email, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, video_url_2, video_url_3, featured, user_id, status) VALUES (${id}, ${slug}, ${data.playerName}, ${data.position}, ${data.secondaryPosition || null}, ${data.birthYear}, ${data.height || null}, ${data.preferredFoot || null}, ${data.currentClub || null}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.level}, ${data.gender}, ${data.gpa || null}, ${data.description || null}, ${data.lookingFor || null}, ${data.contactEmail}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${data.videoUrl2 || null}, ${data.videoUrl3 || null}, false, ${userId}, 'pending')`;
  return slug;
}
...
export async function createPodcastListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO podcasts (id, slug, name, host_name, category, city, country, state, description, website, rss_feed_url, follow_url, email, phone, top_episodes, social_media, logo, image_url, team_photo, image_position, photos, video_url, video_url_2, video_url_3, media_links, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.hostName}, ${data.category}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.description || null}, ${data.website || null}, ${data.rssFeedUrl || null}, ${data.followUrl || null}, ${data.email || null}, ${data.phone || null}, ${data.topEpisodes || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${data.videoUrl2 || null}, ${data.videoUrl3 || null}, ${pf.mediaLinks}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createFacebookGroupListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO facebook_groups (id, slug, name, admin_name, category, group_url, member_count, privacy, city, country, state, description, email, phone, social_media, logo, image_url, team_photo, image_position, photos, video_url, media_links, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.adminName}, ${data.category}, ${data.groupUrl}, ${data.memberCount || null}, ${data.privacy || 'Public'}, ${data.city || ''}, ${data.country || ''}, ${data.state || ''}, ${data.description || null}, ${data.email || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.mediaLinks}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function updateListing(type: string, id: string, data: Record<string, string>, userId: string): Promise<boolean> {
  ...
  switch (type) {
    case "club":
      rows = await sql`UPDATE clubs SET name=${data.name}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, level=${data.level}, league=${data.league || null}, league_url=${data.leagueUrl || null}, age_groups=${data.ageGroups}, gender=${data.gender}, team_count=${Number(data.teamCount) || 0}, description=${data.description}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, media_links=${pf.mediaLinks}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "team":
      rows = await sql`UPDATE teams SET name=${data.name}, club_name=${data.clubName || null}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, level=${data.level}, age_group=${data.ageGroup}, gender=${data.gender}, coach=${data.coach}, looking_for_players=${data.lookingForPlayers === "true"}, positions_needed=${data.positionsNeeded || null}, season=${data.season}, description=${data.description || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, events=${data.events || null}, annual_tournaments=${data.annualTournaments || null}, media_links=${pf.mediaLinks}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "trainer":
      rows = await sql`UPDATE trainers SET name=${data.name}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, specialty=${data.specialty}, experience=${data.experience}, credentials=${data.credentials}, price_range=${data.priceRange}, service_area=${data.serviceArea}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "camp":
      rows = await sql`UPDATE camps SET name=${data.name}, organizer_name=${data.organizerName}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, camp_type=${data.campType}, age_range=${data.ageRange}, dates=${data.dates}, price=${data.price}, gender=${data.gender}, location=${data.location || null}, description=${data.description}, registration_url=${data.registrationUrl || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "guest":
      rows = await sql`UPDATE guest_opportunities SET team_name=${data.teamName}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, level=${data.level}, age_group=${data.ageGroup}, gender=${data.gender}, dates=${data.dates}, tournament=${data.tournament}, positions_needed=${data.positionsNeeded}, contact_email=${data.contactEmail}, description=${data.description || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "tournament":
      rows = await sql`UPDATE tournaments SET name=${data.name}, organizer=${data.organizer}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, dates=${data.dates}, age_groups=${data.ageGroups}, gender=${data.gender}, level=${data.level}, entry_fee=${data.entryFee}, format=${data.format}, description=${data.description}, registration_url=${data.registrationUrl || null}, email=${data.email || null}, region=${data.region || 'US'}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "futsal":
      rows = await sql`UPDATE futsal_teams SET name=${data.name}, club_name=${data.clubName || null}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, level=${data.level}, age_group=${data.ageGroup}, gender=${data.gender}, coach=${data.coach}, looking_for_players=${data.lookingForPlayers === "true"}, positions_needed=${data.positionsNeeded || null}, season=${data.season}, description=${data.description || null}, format=${data.format || '5v5'}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "trip":
      rows = await sql`UPDATE international_trips SET trip_name=${data.tripName}, organizer=${data.organizer}, destination=${data.destination}, city=${data.city}, country=${data.country || 'International'}, state=${data.state || ''}, dates=${data.dates}, age_group=${data.ageGroup}, gender=${data.gender}, level=${data.level}, price=${data.price || null}, spots_available=${data.spotsAvailable || null}, contact_email=${data.contactEmail}, description=${data.description || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "marketplace":
      rows = await sql`UPDATE marketplace SET name=${data.name}, category=${data.category}, description=${data.description}, price=${data.price}, condition=${data.condition}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, contact_email=${data.contactEmail}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, image_position=${Number(data.imagePosition) || 50}, photos=${data.photos || null} WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "player":
      rows = await sql`UPDATE player_profiles SET player_name=${data.playerName}, position=${data.position}, secondary_position=${data.secondaryPosition || null}, birth_year=${data.birthYear}, height=${data.height || null}, preferred_foot=${data.preferredFoot || null}, current_club=${data.currentClub || null}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, level=${data.level}, gender=${data.gender}, gpa=${data.gpa || null}, description=${data.description || null}, looking_for=${data.lookingFor || null}, contact_email=${data.contactEmail}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, video_url_2=${data.videoUrl2 || null}, video_url_3=${data.videoUrl3 || null} WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "podcast":
      rows = await sql`UPDATE podcasts SET name=${data.name}, host_name=${data.hostName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, description=${data.description || null}, website=${data.website || null}, rss_feed_url=${data.rssFeedUrl || null}, follow_url=${data.followUrl || null}, email=${data.email || null}, phone=${data.phone || null}, top_episodes=${data.topEpisodes || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, video_url_2=${data.videoUrl2 || null}, video_url_3=${data.videoUrl3 || null}, media_links=${pf.mediaLinks}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      break;
    case "fbgroup":
...
export async function updateListingAdmin(type: string, id: string, data: Record<string, string>): Promise<boolean> {
  ...
  switch (type) {
    ...
    case "trip":
      ...
      break;
    case "marketplace":
      rows = await sql`UPDATE marketplace SET name=${data.name}, category=${data.category}, description=${data.description}, price=${data.price}, condition=${data.condition}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, contact_email=${data.contactEmail}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, image_position=${Number(data.imagePosition) || 50}, photos=${data.photos || null} WHERE id=${id} RETURNING id`;
      break;
    case "player":
