...
export async function getConsultants() {
  const rows = await sql`SELECT * FROM consultants WHERE status = 'approved' ORDER BY featured DESC, name ASC`;
  return rows.map(mapConsultant);
}

export async function getConsultantBySlug(slug: string) {
  const rows = await sql`SELECT * FROM consultants WHERE slug = ${slug} AND status = 'approved' LIMIT 1`;
  return rows[0] ? mapConsultant(rows[0]) : null;
}

export async function getConsultantSlugs(): Promise<string[]> {
  const rows = await sql`SELECT slug FROM consultants WHERE status = 'approved'`;
  return rows.map((r) => r.slug as string);
}

function mapConsultant(r: Record<string, unknown>) {
  return {
    id: r.id as string, slug: r.slug as string, name: r.name as string,
    city: r.city as string, state: r.state as string, country: r.country as string | undefined,
    specialty: r.specialty as string, experience: r.experience as string,
...
function mapMarketplaceItem(r: Record<string, unknown>): MarketplaceItem {
  ...
  return {
    ...
    imageUrl: r.image_url as string | undefined,
    previewImage: r.preview_image as string | undefined,
    heroImagePosition: typeof r.hero_image_position === "number" ? r.hero_image_position : undefined,
    photos,
...
export async function getAllListings() {
  const [clubs, teams, trainers, recruiters, consultants, camps, guests, tournaments, futsals, trips, marketplace, players, podcasts, fbgroups, services, tryouts, specialEvents, trainingApps, blogRows, youtubeRows, scrimmageRows, soccerBookRows, photoVideoRows] = await Promise.all([
    ...
    sql`SELECT id, slug, name, status, featured, user_id, created_at, 'consultant' as type FROM consultants ORDER BY created_at DESC`,
  ...
  return [...clubs, ...teams, ...trainers, ...recruiters, ...consultants, ...camps, ...guests, ...tournaments, ...futsals, ...trips, ...marketplace, ...players, ...podcasts, ...fbgroups, ...services, ...tryouts, ...specialEvents, ...trainingApps, ...blogRows, ...youtubeRows, ...scrimmageRows, ...soccerBookRows, ...photoVideoRows].map((r) => ({
    id: r.id as string,
...
export async function updateListingStatus(type: string, id: string, status: string): Promise<void> {
  ...
  switch (type) {
    ...
    case "consultant": await sql`UPDATE consultants SET status = ${status} WHERE id = ${id}`; break;
...
export async function updateListingFeatured(type: string, id: string, featured: boolean): Promise<void> {
  ...
  switch (type) {
    ...
    case "consultant": await sql`UPDATE consultants SET featured = ${featured} WHERE id = ${id}`; break;
...
const TYPE_TO_TABLE: Record<string, string> = {
  ...
  scrimmage: "scrimmages", soccerbook: "books", photovideo: "photo_video_services", consultant: "consultants",
};

const FIELD_TO_COLUMN: Record<string, string> = {
  name: "name", description: "description", tagline: "tagline",
  aboutAuthor: "about_author", authorImage: "author_image", link2Label: "link2_label", link2Url: "link2_url", link3Label: "link3_label", link3Url: "link3_url", appStoreUrl: "app_store_url", googlePlayUrl: "google_play_url", price: "price", website: "website",
...
export async function getListingsByUserId(userId: string) {
  const [clubRows, teamRows, trainerRows, consultantRows, recruiterRows, campRows, guestRows, tournamentRows, futsalRows, tripRows, marketplaceRows, playerRows, podcastRows, fbgroupRows, instagramRows, tiktokRows, serviceRows, tryoutRows, specialEventRows, trainingAppRows, blogRows, youtubeRows, fundraiserRows, scrimmageRows, soccerBookRows, photoVideoRows] = await Promise.all([
    ...
    sql`SELECT id, slug, name, status, 'consultant' as type FROM consultants WHERE user_id = ${userId} ORDER BY created_at DESC`,
  ...
  return [...clubRows, ...teamRows, ...trainerRows, ...consultantRows, ...recruiterRows, ...campRows, ...guestRows, ...tournamentRows, ...futsalRows, ...tripRows, ...marketplaceRows, ...playerRows, ...podcastRows, ...fbgroupRows, ...instagramRows, ...tiktokRows, ...serviceRows, ...tryoutRows, ...specialEventRows, ...trainingAppRows, ...blogRows, ...youtubeRows, ...fundraiserRows, ...scrimmageRows, ...soccerBookRows, ...photoVideoRows] as { id: string; slug: string; name: string; status: string; type: string }[];
}
...
export async function createConsultantListing(data: Record<string, string>, userId: string) {
  const id = genId();
  const slug = slugify(data.name);
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO consultants (id, slug, name, city, country, state, specialty, experience, credentials, price_range, service_area, description, phone, social_media, logo, image_url, team_photo, image_position, hero_image_position, photos, video_url, practice_schedule, address, rating, review_count, sponsors, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.specialty}, ${data.experience}, ${data.credentials}, ${data.priceRange}, ${data.serviceArea}, ${data.description || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.heroImagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.practiceSchedule}, ${pf.address}, 0, 0, ${pf.sponsors}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createMarketplaceListing(data: Record<string, string>, userId: string) {
  const id = genId();
  const slug = slugify(data.name);
  await sql`INSERT INTO marketplace (id, slug, name, category, description, price, condition, city, country, state, contact_email, phone, image_url, video_url, image_position, hero_image_position, photos, about_author, sponsors, extra_videos, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.category}, ${data.description}, ${data.price}, ${data.condition}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.contactEmail}, ${data.phone || null}, ${data.imageUrl || null}, ${data.videoUrl || null}, ${Number(data.imagePosition) || 50}, ${Number(data.heroImagePosition) || 50}, ${parsePhotos(data.photos)}, ${data.aboutAuthor || null}, ${data.sponsors || null}, ${data.extraVideos || null}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createPhotoVideoServiceListing(data: Record<string, string>, userId: string) {
  ...
  const slug = slugify(data.name);
  const pf = profileFields(data);
  await sql`INSERT INTO photo_video_services (id, slug, name, provider_name, category, city, country, state, price, description, website, email, phone, image_url, video_url, image_position, hero_image_position, photos, tagline, sponsors, about_author, announcement_heading, announcement_text, announcement_image, announcement_cta, announcement_cta_url, announcement_heading_2, announcement_text_2, announcement_image_2, announcement_cta_2, announcement_cta_url_2, announcement_heading_3, announcement_text_3, announcement_image_3, announcement_cta_3, announcement_cta_url_3, extra_videos, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.providerName}, ${data.category}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.price || null}, ${data.description || null}, ${data.website || null}, ${data.email || null}, ${data.phone || null}, ${data.imageUrl || null}, ${data.videoUrl || null}, ${Number(data.imagePosition) || 50}, ${Number(data.heroImagePosition) || 50}, ${pf.photos}, ${pf.tagline}, ${pf.sponsors}, ${data.aboutAuthor || null}, ${data.announcementHeading || null}, ${data.announcementText || null}, ${data.announcementImage || null}, ${data.announcementCta || null}, ${data.announcementCtaUrl || null}, ${data.announcementHeading2 || null}, ${data.announcementText2 || null}, ${data.announcementImage2 || null}, ${data.announcementCta2 || null}, ${data.announcementCtaUrl2 || null}, ${data.announcementHeading3 || null}, ${data.announcementText3 || null}, ${data.announcementImage3 || null}, ${data.announcementCta3 || null}, ${data.announcementCtaUrl3 || null}, ${data.extraVideos || null}, false, ${userId}, 'approved')`;
  return slug;
}
...
function mapSoccerBook(r: Record<string, unknown>): SoccerBook {
  let mediaAppearances: SoccerBook["mediaAppearances"];
  ...
  return {
    ...
    link2Label: r.link2_label as string | undefined,
...
export async function getListingData(type: string, id: string, userId: string): Promise<Record<string, string> | null> {
  ...
  switch (type) {
    ...
    case "consultant":
      rows = await sql`SELECT * FROM consultants WHERE id = ${id} AND user_id = ${userId} LIMIT 1`;
      if (!rows[0]) return null;
      return mapConsultantToForm(rows[0]);
    case "recruiter":
...
export async function getListingDataAdmin(type: string, id: string): Promise<Record<string, string> | null> {
  ...
  switch (type) {
    ...
    case "consultant": rows = await sql`SELECT * FROM consultants WHERE id = ${id} LIMIT 1`; break;
  ...
  switch (type) {
    ...
    case "consultant": return mapConsultantToForm(rows[0]);
...
function parseSocial(raw: unknown): { facebook: string; instagram: string; youtube: string; linkedin: string; twitter: string } {
  if (!raw) return { facebook: "", instagram: "", youtube: "", linkedin: "", twitter: "" };
  try {
    const sm = typeof raw === "string" ? JSON.parse(raw) : raw;
    return { facebook: sm.facebook || "", instagram: sm.instagram || "", youtube: sm.youtube || "", linkedin: sm.linkedin || "", twitter: sm.twitter || "" };
  } catch { return { facebook: "", instagram: "", youtube: "", linkedin: "", twitter: "" }; }
}
...
function mapServiceToForm(r: Record<string, unknown>): Record<string, string> {
  const sm = parseSocial(r.social_media);
  return { slug: s(r.slug), name: s(r.name), providerName: s(r.provider_name), category: s(r.category), city: s(r.city), country: s(r.country) || "United States", state: s(r.state), price: s(r.price), description: s(r.description), website: s(r.website), email: s(r.email), phone: s(r.phone), facebook: sm.facebook, instagram: sm.instagram, linkedin: sm.linkedin, twitter: sm.twitter, imageUrl: s(r.image_url), videoUrl: s(r.video_url), photos: s(r.photos), announcementHeading: s(r.announcement_heading), announcementText: s(r.announcement_text), announcementImage: s(r.announcement_image), announcementCta: s(r.announcement_cta), announcementCtaUrl: s(r.announcement_cta_url), announcementHeading2: s(r.announcement_heading_2), announcementText2: s(r.announcement_text_2), announcementImage2: s(r.announcement_image_2), announcementCta2: s(r.announcement_cta_2), announcementCtaUrl2: s(r.announcement_cta_url_2), announcementHeading3: s(r.announcement_heading_3), announcementText3: s(r.announcement_text_3), announcementImage3: s(r.announcement_image_3), announcementCta3: s(r.announcement_cta_3), announcementCtaUrl3: s(r.announcement_cta_url_3), aboutAuthor: s(r.about_author), tagline: s(r.tagline) };
}
...
export async function updateListing(type: string, id: string, data: Record<string, string>, userId: string): Promise<boolean> {
  ...
  switch (type) {
    ...
    case "consultant": {
      const newSlug = data.slug ? await resolveListingSlug("consultant", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE consultants SET slug=${newSlug}, name=${data.name}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, specialty=${data.specialty}, experience=${data.experience}, credentials=${data.credentials}, price_range=${data.priceRange}, service_area=${data.serviceArea}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
        rows = await sql`UPDATE consultants SET name=${data.name}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, specialty=${data.specialty}, experience=${data.experience}, credentials=${data.credentials}, price_range=${data.priceRange}, service_area=${data.serviceArea}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      }
      break;
    }
    case "recruiter": {
      const newSlug = data.slug ? await resolveListingSlug("recruiter", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE recruiters SET slug=${newSlug}, name=${data.name}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, specialty=${data.specialty || null}, experience=${data.experience || null}, credentials=${data.credentials || null}, price_range=${data.priceRange || null}, service_area=${data.serviceArea || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, practice_schedule=${pf.practiceSchedule}, address=${pf.address}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
    ...
    case "tournament": {
      const newSlug = data.slug ? await resolveListingSlug("tournament", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE tournaments SET slug=${newSlug}, name=${data.name}, organizer=${data.organizer}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, dates=${data.dates}, age_groups=${data.ageGroups}, gender=${data.gender}, level=${data.level}, entry_fee=${data.entryFee}, format=${data.format}, description=${data.description}, registration_url=${data.registrationUrl || null}, email=${data.email || null}, region=${data.region || 'US'}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
    ...
    case "trip": {
      const newSlug = data.slug ? await resolveListingSlug("trip", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE international_trips SET slug=${newSlug}, trip_name=${data.tripName}, organizer=${data.organizer}, destination=${data.destination}, city=${data.city}, country=${data.country || 'International'}, state=${data.state || ''}, dates=${data.dates}, age_group=${data.ageGroup}, gender=${data.gender}, level=${data.level}, price=${data.price || null}, spots_available=${data.spotsAvailable || null}, contact_email=${data.contactEmail}, description=${data.description || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, sponsors=${pf.sponsors}, staff_members=${data.staffMembers || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
    ...
    case "marketplace": {
      const newSlug = data.slug ? await resolveListingSlug("marketplace", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE marketplace SET slug=${newSlug}, name=${data.name}, category=${data.category}, description=${data.description}, price=${data.price}, condition=${data.condition}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, contact_email=${data.contactEmail}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, preview_image=${data.previewImage || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, about_author=${data.aboutAuthor || null}, sponsors=${data.sponsors || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
        rows = await sql`UPDATE marketplace SET name=${data.name}, category=${data.category}, description=${data.description}, price=${data.price}, condition=${data.condition}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, contact_email=${data.contactEmail}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, preview_image=${data.previewImage || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, about_author=${data.aboutAuthor || null}, sponsors=${data.sponsors || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      }
      break;
    }
    case "player": {
      const newSlug = data.slug ? await resolveListingSlug("player", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE player_profiles SET slug=${newSlug}, player_name=${data.playerName}, position=${data.position}, secondary_position=${data.secondaryPosition || null}, birth_year=${data.birthYear}, birth_month=${data.birthMonth || null}, height=${data.height || null}, preferred_foot=${data.preferredFoot || null}, current_club=${data.currentClub || null}, league=${data.league || null}, team_name=${data.teamName || null}, favorite_team=${data.favoriteTeam || null}, favorite_player=${data.favoritePlayer || null}, game_highlights=${data.gameHighlights || null}, highlight_videos=${data.highlightVideos || '[]'}, cv_url=${data.cvUrl || null}, available_for_guest_play=${data.availableForGuestPlay === "true"}, looking_for_team=${data.lookingForTeam === "true"}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, level=${data.level}, gender=${data.gender}, gpa=${data.gpa || null}, description=${data.description || null}, looking_for=${data.lookingFor || null}, contact_email=${data.contactEmail}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, video_url_2=${data.videoUrl2 || null}, video_url_3=${data.videoUrl3 || null}, sponsors=${data.sponsors || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
    ...
    case "tiktokpage": {
      const newSlug = data.slug ? await resolveListingSlug("tiktokpage", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE tiktok_pages SET slug=${newSlug}, name=${data.name}, owner_name=${data.ownerName}, category=${data.category}, page_url=${data.pageUrl}, follower_count=${data.followerCount || null}, privacy=${data.privacy || 'Public'}, city=${data.city || ''}, country=${data.country || ''}, state=${data.state || ''}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, tagline=${data.tagline || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, video_url_2=${data.videoUrl2 || null}, video_url_3=${data.videoUrl3 || null}, media_links=${pf.mediaLinks}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
    ...
    case "trainingapp": {
      const newSlug = data.slug ? await resolveListingSlug("trainingapp", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE training_apps SET slug=${newSlug}, name=${data.name}, provider_name=${data.providerName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, price=${data.price || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_cta=${data.announcementCta || null}, announcement_cta_url=${data.announcementCtaUrl || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_cta_2=${data.announcementCta2 || null}, announcement_cta_url_2=${data.announcementCtaUrl2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, announcement_cta_3=${data.announcementCta3 || null}, announcement_cta_url_3=${data.announcementCtaUrl3 || null}, tagline=${data.tagline || null}, about_author=${data.aboutAuthor || null}, app_store_url=${data.appStoreUrl || null}, google_play_url=${data.googlePlayUrl || null}, social_media=${sm}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} AND user_id=${userId} RETURNING id`;
      } else {
...
export async function updateListingAdmin(type: string, id: string, data: Record<string, string>): Promise<boolean> {
  ...
  switch (type) {
    ...
    case "trip": {
      const newSlug = data.slug ? await resolveListingSlug("trip", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE international_trips SET slug=${newSlug}, trip_name=${data.tripName}, organizer=${data.organizer}, destination=${data.destination}, city=${data.city}, country=${data.country || 'International'}, state=${data.state || ''}, dates=${data.dates}, age_group=${data.ageGroup}, gender=${data.gender}, level=${data.level}, price=${data.price || null}, spots_available=${data.spotsAvailable || null}, contact_email=${data.contactEmail}, description=${data.description || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, tagline=${pf.tagline}, preview_image=${pf.previewImage}, sponsors=${pf.sponsors}, staff_members=${data.staffMembers || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      } else {
        rows = await sql`UPDATE international_trips SET trip_name=${data.tripName}, organizer=${data.organizer}, destination=${data.destination}, city=${data.city}, country=${data.country || 'International'}, state=${data.state || ''}, dates=${data.dates}, age_group=${data.ageGroup}, gender=${data.gender}, level=${data.level}, price=${data.price || null}, spots_available=${data.spotsAvailable || null}, contact_email=${data.contactEmail}, description=${data.description || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, tagline=${pf.tagline}, preview_image=${pf.previewImage}, sponsors=${pf.sponsors}, staff_members=${data.staffMembers || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      }
      break;
    ...
    case "marketplace": {
      const newSlug = data.slug ? await resolveListingSlug("marketplace", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE marketplace SET slug=${newSlug}, name=${data.name}, category=${data.category}, description=${data.description}, price=${data.price}, condition=${data.condition}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, contact_email=${data.contactEmail}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, about_author=${data.aboutAuthor || null}, tagline=${pf.tagline}, preview_image=${pf.previewImage}, sponsors=${data.sponsors || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      } else {
        rows = await sql`UPDATE marketplace SET name=${data.name}, category=${data.category}, description=${data.description}, price=${data.price}, condition=${data.condition}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, contact_email=${data.contactEmail}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, about_author=${data.aboutAuthor || null}, tagline=${pf.tagline}, preview_image=${pf.previewImage}, sponsors=${data.sponsors || null}, extra_videos=${data.extraVideos || null}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      }
      break;
    }
    case "player": {
    ...
    case "service": {
      const newSlug = data.slug ? await resolveListingSlug("service", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE services SET slug=${newSlug}, name=${data.name}, provider_name=${data.providerName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, price=${data.price || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_cta=${data.announcementCta || null}, announcement_cta_url=${data.announcementCtaUrl || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_cta_2=${data.announcementCta2 || null}, announcement_cta_url_2=${data.announcementCtaUrl2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, announcement_cta_3=${data.announcementCta3 || null}, announcement_cta_url_3=${data.announcementCtaUrl3 || null}, tagline=${pf.tagline}, about_author=${data.aboutAuthor || null}, app_store_url=${data.appStoreUrl || null}, google_play_url=${data.googlePlayUrl || null}, social_media=${sm}, preview_image=${pf.previewImage}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      } else {
        rows = await sql`UPDATE services SET name=${data.name}, provider_name=${data.providerName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, price=${data.price || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_cta=${data.announcementCta || null}, announcement_cta_url=${data.announcementCtaUrl || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_cta_2=${data.announcementCta2 || null}, announcement_cta_url_2=${data.announcementCtaUrl2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, announcement_cta_3=${data.announcementCta3 || null}, announcement_cta_url_3=${data.announcementCtaUrl3 || null}, tagline=${pf.tagline}, about_author=${data.aboutAuthor || null}, app_store_url=${data.appStoreUrl || null}, google_play_url=${data.googlePlayUrl || null}, social_media=${sm}, preview_image=${pf.previewImage}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      }
      break;
    ...
    case "trainingapp": {
      const newSlug = data.slug ? await resolveListingSlug("trainingapp", data.slug, id) : null;
      if (newSlug) {
        rows = await sql`UPDATE training_apps SET slug=${newSlug}, name=${data.name}, provider_name=${data.providerName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, price=${data.price || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_cta=${data.announcementCta || null}, announcement_cta_url=${data.announcementCtaUrl || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_cta_2=${data.announcementCta2 || null}, announcement_cta_url_2=${data.announcementCtaUrl2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, announcement_cta_3=${data.announcementCta3 || null}, announcement_cta_url_3=${data.announcementCtaUrl3 || null}, tagline=${data.tagline || null}, about_author=${data.aboutAuthor || null}, app_store_url=${data.appStoreUrl || null}, google_play_url=${data.googlePlayUrl || null}, social_media=${sm}, tagline=${pf.tagline}, preview_image=${pf.previewImage}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      } else {
        rows = await sql`UPDATE training_apps SET name=${data.name}, provider_name=${data.providerName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, price=${data.price || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${parsePhotos(data.photos)}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_cta=${data.announcementCta || null}, announcement_cta_url=${data.announcementCtaUrl || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_cta_2=${data.announcementCta2 || null}, announcement_cta_url_2=${data.announcementCtaUrl2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, announcement_cta_3=${data.announcementCta3 || null}, announcement_cta_url_3=${data.announcementCtaUrl3 || null}, tagline=${data.tagline || null}, about_author=${data.aboutAuthor || null}, app_store_url=${data.appStoreUrl || null}, google_play_url=${data.googlePlayUrl || null}, social_media=${sm}, tagline=${pf.tagline}, preview_image=${pf.previewImage}, sponsors=${pf.sponsors}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      }
      break;
...
export async function archiveListing(type: string, id: string, userId: string, isAdmin = false): Promise<boolean> {
  ...
  if (isAdmin) {
    switch (type) {
      ...
      case "consultant": rows = await sql`UPDATE consultants SET status = 'archived', updated_at = NOW() WHERE id = ${id} RETURNING id`; break;
    ...
    switch (type) {
      ...
      case "consultant": rows = await sql`UPDATE consultants SET status = 'archived', updated_at = NOW() WHERE id = ${id} AND user_id = ${userId} RETURNING id`; break;
...
export async function restoreListing(type: string, id: string, userId: string, isAdmin = false): Promise<boolean> {
  ...
  if (isAdmin) {
    switch (type) {
      ...
      case "consultant": rows = await sql`UPDATE consultants SET status = 'approved', updated_at = NOW() WHERE id = ${id} RETURNING id`; break;
    ...
    switch (type) {
      ...
      case "consultant": rows = await sql`UPDATE consultants SET status = 'approved', updated_at = NOW() WHERE id = ${id} AND user_id = ${userId} RETURNING id`; break;
...
export async function deleteListing(type: string, id: string, userId: string, isAdmin = false): Promise<boolean> {
  ...
  if (isAdmin) {
    switch (type) {
      ...
      case "consultant": rows = await sql`DELETE FROM consultants WHERE id = ${id} RETURNING id`; break;
    ...
    switch (type) {
      ...
      case "consultant": rows = await sql`DELETE FROM consultants WHERE id = ${id} AND user_id = ${userId} RETURNING id`; break;
...
export async function getListingContact(type: string, slug: string): Promise<{ name: string; email: string | null; userId: string } | null> {
  ...
  switch (type) {
    ...
    case "consultant": rows = await sql`SELECT name, email, user_id FROM consultants WHERE slug = ${slug} LIMIT 1`; break;
...
export async function getListingOwner(type: string, slug: string): Promise<string | null> {
  ...
  switch (type) {
    ...
    case "consultant": rows = await sql`SELECT user_id FROM consultants WHERE slug = ${slug} LIMIT 1`; break;
...
export async function getListingOwnerIdById(type: string, id: string): Promise<string | null> {
  ...
  switch (t) {
    ...
    case "consultant": rows = await sql`SELECT user_id FROM consultants WHERE id = ${id} LIMIT 1`; break;
...
export async function getListingSlugById(type: string, id: string): Promise<string | null> {
  ...
  switch (t) {
    ...
    case "consultant": rows = await sql`SELECT slug FROM consultants WHERE id = ${id} LIMIT 1`; break;
...
export async function getListingNameById(type: string, id: string): Promise<string | null> {
  ...
  switch (type) {
    ...
    case "consultant": rows = await sql`SELECT name FROM consultants WHERE id = ${id} LIMIT 1`; break;
...
async function resolveListingSlug(type: string, slug: string, currentId: string): Promise<string | null> {
  if (!slug) return null;
  const clean = sanitizeSlug(slug);
  if (!clean) return null;
  let existing: Record<string, unknown>[];
  switch (type) {
    case "club": existing = await sql`SELECT id FROM clubs WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "team": existing = await sql`SELECT id FROM teams WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "trainer": existing = await sql`SELECT id FROM trainers WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "consultant": existing = await sql`SELECT id FROM consultants WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "recruiter": existing = await sql`SELECT id FROM recruiters WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "camp": existing = await sql`SELECT id FROM camps WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "tournament": existing = await sql`SELECT id FROM tournaments WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "futsal": existing = await sql`SELECT id FROM futsal_teams WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "guest": existing = await sql`SELECT id FROM guest_opportunities WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "podcast": existing = await sql`SELECT id FROM podcasts WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "tryout": existing = await sql`SELECT id FROM tryouts WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "specialevent": existing = await sql`SELECT id FROM special_events WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "scrimmage": existing = await sql`SELECT id FROM scrimmages WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "blog": existing = await sql`SELECT id FROM blogs WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "youtube": existing = await sql`SELECT id FROM youtube_channels WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "trainingapp": existing = await sql`SELECT id FROM training_apps WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "service": existing = await sql`SELECT id FROM services WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "soccerbook": existing = await sql`SELECT id FROM books WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "photovideo": existing = await sql`SELECT id FROM photo_video_services WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "fbgroup": existing = await sql`SELECT id FROM facebook_groups WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "instagrampage": existing = await sql`SELECT id FROM instagram_pages WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "tiktokpage": existing = await sql`SELECT id FROM tiktok_pages WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "marketplace": existing = await sql`SELECT id FROM marketplace WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
    case "trip": existing = await sql`SELECT id FROM international_trips WHERE slug = ${clean} AND id != ${currentId} LIMIT 1`; break;
...
export interface PodcastEpisodeLink {
  label: string;
  url: string;
}
...
export async function updatePodcastEpisode(id: string, data: { title?: string; description?: string; embedUrl?: string | null; embedHtml?: string | null; slug?: string; previewImage?: string; links?: PodcastEpisodeLink[] }): Promise<boolean> {
  // Add links column if it doesn't exist (safe to run repeatedly)
  await sql`ALTER TABLE podcast_episodes ADD COLUMN IF NOT EXISTS links JSONB`;
  const linksJson = data.links ? JSON.stringify(data.links) : null;
  const rows = await sql`UPDATE podcast_episodes SET
    title = ${data.title ?? null},
    description = ${data.description ?? null},
    slug = COALESCE(${data.slug || null}, slug),
    embed_url = ${data.embedUrl ?? null},
    embed_html = ${data.embedHtml ?? null},
    preview_image = COALESCE(${data.previewImage || null}, preview_image),
    links = ${linksJson}
