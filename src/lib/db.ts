...
export async function createPodcastListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO podcasts (id, slug, name, host_name, category, city, country, state, description, website, rss_feed_url, follow_url, email, phone, top_episodes, social_media, logo, image_url, team_photo, image_position, hero_image_position, photos, video_url, video_url_2, video_url_3, media_links, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.hostName}, ${data.category}, ${data.city}, ${data.country || 'United States'}, ${data.state}, ${data.description || null}, ${data.website || null}, ${data.rssFeedUrl || null}, ${data.followUrl || null}, ${data.email || null}, ${data.phone || null}, ${data.topEpisodes || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.heroImagePosition}, ${pf.photos}, ${pf.videoUrl}, ${data.videoUrl2 || null}, ${data.videoUrl3 || null}, ${pf.mediaLinks}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function createFacebookGroupListing(data: Record<string, string>, userId: string) {
  ...
  const sm = buildSocialMedia(data);
  const pf = profileFields(data);
  await sql`INSERT INTO facebook_groups (id, slug, name, admin_name, category, group_url, member_count, privacy, city, country, state, description, email, phone, social_media, logo, image_url, team_photo, image_position, hero_image_position, photos, video_url, media_links, featured, user_id, status) VALUES (${id}, ${slug}, ${data.name}, ${data.adminName}, ${data.category}, ${data.groupUrl}, ${data.memberCount || null}, ${data.privacy || 'Public'}, ${data.city || ''}, ${data.country || ''}, ${data.state || ''}, ${data.description || null}, ${data.email || null}, ${data.phone || null}, ${sm}, ${data.logo || null}, ${data.imageUrl || null}, ${pf.teamPhoto}, ${pf.imagePosition}, ${pf.heroImagePosition}, ${pf.photos}, ${pf.videoUrl}, ${pf.mediaLinks}, false, ${userId}, 'approved')`;
  return slug;
}
...
export async function updateListingAdmin(type: string, id: string, data: Record<string, string>): Promise<boolean> {
  ...
  switch (type) {
    ...
    case "fbgroup":
      ...
      break;
    case "service":
      rows = await sql`UPDATE services SET name=${data.name}, provider_name=${data.providerName}, category=${data.category}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, price=${data.price || null}, description=${data.description || null}, website=${data.website || null}, email=${data.email || null}, phone=${data.phone || null}, image_url=${data.imageUrl || null}, video_url=${data.videoUrl || null}, image_position=${Number(data.imagePosition) || 50}, hero_image_position=${Number(data.heroImagePosition) || 50}, photos=${data.photos || null}, announcement_heading=${data.announcementHeading || null}, announcement_text=${data.announcementText || null}, announcement_image=${data.announcementImage || null}, announcement_heading_2=${data.announcementHeading2 || null}, announcement_text_2=${data.announcementText2 || null}, announcement_image_2=${data.announcementImage2 || null}, announcement_heading_3=${data.announcementHeading3 || null}, announcement_text_3=${data.announcementText3 || null}, announcement_image_3=${data.announcementImage3 || null}, about_author=${data.aboutAuthor || null}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      break;
    case "tryout":
      rows = await sql`UPDATE tryouts SET name=${data.name}, organizer_name=${data.organizerName}, club_name=${data.clubName || null}, city=${data.city}, country=${data.country || 'United States'}, state=${data.state}, tryout_type=${data.tryoutType}, age_group=${data.ageGroup}, gender=${data.gender}, dates=${data.dates}, time=${data.time || null}, location=${data.location || null}, cost=${data.cost || null}, description=${data.description}, registration_url=${data.registrationUrl || null}, email=${data.email || null}, phone=${data.phone || null}, social_media=${sm}, logo=${data.logo || null}, image_url=${data.imageUrl || null}, team_photo=${pf.teamPhoto}, image_position=${pf.imagePosition}, hero_image_position=${pf.heroImagePosition}, photos=${pf.photos}, video_url=${pf.videoUrl}, updated_at=NOW() WHERE id=${id} RETURNING id`;
      break;
