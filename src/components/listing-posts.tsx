...
export function ListingPostsSidebar({
  ...
  const blogPosts = posts.filter((p) => p.title);
  const regularPosts = posts.filter((p) => !p.title);

  return (
    <>
    {/* Our Blog section */}
    {(blogPosts.length > 0 || canManage) && (
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden mb-3">
              ...
              <div className="flex items-start gap-2">
                ...
                {canManage && (
                  <div className="flex gap-1 shrink-0 pt-0.5">
                    <button onClick={() => handleToggleHidden(post.id)} className="text-muted hover:text-primary transition-colors" title={post.hidden ? "Show" : "Hide"}>
                      {post.hidden ? (
                    ...
                    <button onClick={() => handleDelete(post.id)} className="text-muted hover:text-red-500 transition-colors" title="Delete">
        ...
        {blogPosts.length === 0 && canManage && (
          <div className="px-4 pb-3.5 text-center">
            <p className="text-xs text-muted">No blog posts yet</p>
          </div>
        )}
      </div>
    )}
    ...
    {/* Our Posts section */}
    {(regularPosts.length > 0 || canManage) && (
    <div id="create-post" className="bg-white rounded-2xl shadow-sm overflow-hidden">
      ...
      {canManage && showForm && (
        <form onSubmit={handleSubmit} className="px-4 pb-3 space-y-2.5">
          ...
          <input
            ...
            placeholder="YouTube, Shorts, Vimeo, or Instagram URL (optional)"
      ...
      {regularPosts.length === 0 && canManage && !showForm && (
        <div className="px-4 pb-3.5 text-center">
